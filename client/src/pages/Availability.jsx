...
export default function Availability() {
  const [schedule, setSchedule] = useState(base); const [leave, setLeave] = useState({ from: '', to: '' }); const [leaveError, setLeaveError] = useState('');
  const [special, setSpecial] = useState({ date: '', start: '09:00', end: '17:00' }); const [specialError, setSpecialError] = useState(''); const [error, setError] = useState(''); const [saved, setSaved] = useState(false);
  ...
  // One date worked at different hours than that weekday usually runs. It is
  // stored as an open override, which the slot grid reads instead of the week.
  const addSpecialHours = () => {
    if (special.end <= special.start) return setSpecialError('The closing time has to be after the opening time.');
    const entry = { date: special.date, enabled: true, start: special.start, end: special.end, breaks: [] };
    const kept = schedule.overrides.filter((item) => item.date !== entry.date);
    setSchedule({ ...schedule, overrides: [...kept, entry].sort((a, b) => a.date.localeCompare(b.date)) });
    setSpecial({ date: '', start: '09:00', end: '17:00' }); setSpecialError('');
  };
  const applyForLeave = () => {
  ...
  const openDates = schedule.overrides.filter((item) => item.enabled !== false);
