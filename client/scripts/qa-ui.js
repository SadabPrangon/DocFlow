...
async function run() {
  ...
  try {
    ...
    check((await page.locator('.dd-option').allInnerTexts()).map(t=>t.trim()).join('|')==='15 minutes|20 minutes|30 minutes|45 minutes|1 hour|1 hour 30 min|2 hours|Custom length', 'Consultation lengths run from 15 minutes to 2 hours, plus a custom entry', (await page.locator('.dd-option').allInnerTexts()).join('|'));
    ...
    check(await page.locator('.cons-minutes').count()===0, 'A preset length keeps the minutes field out of the way');
    await page.locator('.cons .dd-button').click();
    await page.locator('.dd-option',{hasText:'Custom length'}).click();
    await page.locator('.cons-minutes input').fill('25');
    await page.waitForTimeout(150);
    check((await page.locator('.cons-hint').innerText()).includes('fits 19 patients at 25 minutes'), 'A typed consultation length is accepted', await page.locator('.cons-hint').innerText());
    await page.locator('.cons-minutes input').fill('7');
    await page.waitForTimeout(150);
    check((await page.locator('.cons-hint').innerText()).includes('between 15 and 240')&&await page.locator('.cons-minutes input').getAttribute('aria-invalid')==='true', 'A length outside 15 to 240 minutes is refused', await page.locator('.cons-hint').innerText());
