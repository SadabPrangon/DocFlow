...
async function run() {
  ...
  try {
    ...
    check(await leaveCard.locator('.leave-button').isDisabled(), 'Applying for leave needs a date first');
    ...
    await leaveCard.locator('.leave-button').click();
    await page.waitForTimeout(200);
    ...
    const hoursCard=page.locator('section').nth(2);
    check((await hoursCard.locator('h2').innerText()).trim()==='Hours for a single date', 'A third card sets the hours for one date', await hoursCard.locator('h2').innerText());
    await hoursCard.locator('input[type=date]').fill('2026-09-20');
    await hoursCard.locator('input[type=time]').first().fill('16:00');
    await hoursCard.locator('input[type=time]').nth(1).fill('12:00');
    await hoursCard.locator('.leave-button').click();
    await page.waitForTimeout(150);
    check((await hoursCard.locator('.cons-hint').innerText()).includes('after the opening time'), 'Closing before opening is refused', await hoursCard.locator('.cons-hint').innerText());
    await hoursCard.locator('input[type=time]').first().fill('10:00');
    await hoursCard.locator('input[type=time]').nth(1).fill('14:00');
    await hoursCard.locator('.leave-button').click();
    await page.waitForTimeout(150);
    check((await hoursCard.locator('.space-y-2 > div').first().innerText()).replace(/s+/g,' ').includes('2026-09-20 · 10:00–14:00'), 'A date keeps its own opening hours', await hoursCard.locator('.space-y-2').innerText());
    await leaveCard.locator('input[type=date]').first().fill('2026-09-20');
    await leaveCard.locator('.leave-button').click();
    await page.waitForTimeout(150);
    check((await hoursCard.locator('.space-y-2').innerText()).includes('No date has its own hours'), 'Booking leave over a date drops the hours it had', await hoursCard.locator('.space-y-2').innerText());
    check(!(await page.locator('main').innerText()).includes('Availability updated'), 'The schedule page carries no confirmation banner');
    check(await page.locator('.sched-button').count()===3&&(await page.locator('.sched-head').allInnerTexts()).every(text=>text.includes('Save schedule')), 'Every schedule card saves from its own header', (await page.locator('.sched-head').allInnerTexts()).join(' | '));
