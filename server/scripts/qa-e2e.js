...
async function run() {
  try {
    ...
    response=await call('/api/appointments',{method:'POST',token:sessions.patient3,body:{...scheduleBody,reason:'Collision test'}});
    check(response.status===409,'Already-booked doctor time slot is unavailable');
    response=await call(`/api/appointments/${scheduleId}/reschedule`,{method:'PUT',token:sessions.patient4,body:{appointmentDate:'2027-01-15',appointmentTime:'10:00 AM'}});
